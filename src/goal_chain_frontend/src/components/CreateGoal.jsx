...
// for the time input for scheduled goal status type, convience method to get default values relative to now
...
const CreateGoal = () => {
  ...
  const { statusType, title, description, tags, scheduledInterval, minDate } =
    state;

  const { isUpdating, hasCreateError, clearError, createGoal } =
  ...
  return (
    <>
      ...
      <div className="one-rem-padding" />
      <Dropdown
        options={["Unscheduled", "Scheduled", "Active"]}
        value={"Unscheduled"}
